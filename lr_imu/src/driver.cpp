...
class lrImuDriver : public rclcpp::Node
{
  public:
	...
	: Node("lr_imu")
	{	
		RCLCPP_INFO(this->get_logger(), "Link Robotics Imu ROS2 Driver v1.1 started");
		...
		if (config == true) {
			...
			// set data rate
			uint16_t dataRate = 200; 
			if (imuPtr->setDataRate(dataRate) == true) {
				RCLCPP_INFO(this->get_logger(), " Data rate changed to %d", dataRate);
			}
			
			// output setting
			uint16_t mask =  (1 << imuPtr->outputSettingQuaternion) |
					  (1 << imuPtr->outputSettingMagneticField) |
					  (1 << imuPtr->outputSettingCovariance) |
					  (1 << imuPtr->outputSettingLinearAcceleration) |
					  (1 << imuPtr->outputSettingAngularRate) |
					  (1 << imuPtr->outputSettingEulerAngles);
					  
			if (imuPtr->setOutputs(mask) == true) {	
				RCLCPP_INFO(this->get_logger(), " Output setting changed to %u", mask);
			}
			
			RCLCPP_INFO(this->get_logger(), "Saving settings to device ...");
		...
		if (true) {
			...
			if (imuPtr->loadSettings() == true) {
				...
				if (baudRate > 0) {
					RCLCPP_INFO(this->get_logger(), " Baud rate is  %u bps", baudRate);
					
				}
				
				uint16_t dataRate = imuPtr->getDataRate();
				if (dataRate > 0) {
					RCLCPP_INFO(this->get_logger(), " Data rate is %u Hz", dataRate);
  ...
  private:
	boost::shared_ptr<imu> imuPtr;
  	boost::shared_ptr<std::thread> thPtr;
